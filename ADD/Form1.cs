...
namespace ADD
{
    public partial class Form1 : Form
    {
        ...
        {
            FormCompra fcp = new FormCompra();
            var resultado = fcp.ShowDialog();
            if (resultado == DialogResult.OK)
            {
                Compra compra = new Compra();
                compra.idProduto = fcp.Produto;
                compra.idFornecedor = fcp.;
                compra.quantidade = fcp.Telefone;
                compra.precoUt = fcp.NomeContato;
                compra.totalD = fcp.endereco;
